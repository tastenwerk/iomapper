...
var paths = {
  ...
  children: function pathsChildren( query, options, callback ){
    ...
    function runInitChild(){
      ...
      mongoose.models[item._type].findById( item._id ).execWithUser( self.holder, function( err, child ){
        if( child ){
        ...
        if( children.length === 0 )
          ...
          children.forEach( function( child ){
            if( child.paths && child.paths.length === 1 )
              mongoose.models[child._type].findByIdAndRemove( child._id, runNext );
            else
              runNext();
          });
        }
